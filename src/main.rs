...
fn draw(ctx: &mut Context) -> Result<(), Box<dyn Error>> {
    ...
    unsafe {
        ...
        ctx.device.cmd_push_constants(
            ...
            ctx.shader.vert_pipeline_layout,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    */
    ...
    let layout_sampler_binding = ash::vk::DescriptorSetLayoutBinding::default()
        .binding(1u32)
        .descriptor_type(ash::vk::DescriptorType::COMBINED_IMAGE_SAMPLER)
        .descriptor_count(1u32)
        .stage_flags(ash::vk::ShaderStageFlags::FRAGMENT);

    let yo_2 = &[layout_sampler_binding];

    let layout_info = ash::vk::DescriptorSetLayoutCreateInfo::default().bindings(yo_2);
	...
	let descriptor_set = descriptor_sets.unwrap()[0];
